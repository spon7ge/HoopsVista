...
def avg_REB_last_3(player_data, player_id_col='PLAYER_ID', props=['REB'], games=3, date_col='GAME_DATE'):
...
def avg_REB_last_5(player_data, player_id_col='PLAYER_ID', props=['REB'], games=5, date_col='GAME_DATE'):
    # If a single stat is passed, convert it to a list to keep the logic consistent
    if isinstance(props, str):
        props = [props]
...
def avg_REB_last_7(player_data, player_id_col='PLAYER_ID', props=['REB'], games=7, date_col='GAME_DATE'):
    # If a single stat is passed, convert it to a list to keep the logic consistent
    if isinstance(props, str):
        props = [props]
...
def add_REB_home_avg(player_data, props=['REB']): 
...
def add_REB_away_avg(player_data, props=['REB']): 
    # Create a new column for each prop or combination of props
    if isinstance(props, list):  # Case for multiple properties like PTS + AST
