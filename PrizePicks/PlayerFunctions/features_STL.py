...
def avg_STL_last_3(player_data, player_id_col='PLAYER_ID', props=['STL'], games=3, date_col='GAME_DATE'):
...
def avg_STL_last_5(player_data, player_id_col='PLAYER_ID', props=['STL'], games=5, date_col='GAME_DATE'):
    # If a single stat is passed, convert it to a list to keep the logic consistent
    if isinstance(props, str):
        props = [props]
...
def avg_STL_last_7(player_data, player_id_col='PLAYER_ID', props=['STL'], games=7, date_col='GAME_DATE'):
    # If a single stat is passed, convert it to a list to keep the logic consistent
    if isinstance(props, str):
        props = [props]
...
def add_STL_home_avg(player_data, props=['STL']): 
...
def add_STL_away_avg(player_data, props=['STL']): 
    # Create a new column for each prop or combination of props
    if isinstance(props, list):  # Case for multiple properties like PTS + AST
